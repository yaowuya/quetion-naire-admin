  ...
  <div class="app-container">
    <el-form :inline="true" :model="queryForm" class="pl-5">
      <el-form-item label="角色ID">
        <el-input v-model="queryForm.name" placeholder="请输入角色ID" clearable />
      </el-form-item>
      <el-form-item>
...
export default {
  ...
  methods: {
    ...
    async query() {
      this.loading = true
      try {
        const res = await this.$api.sys.getRole({
    ...
    remove(row) {
      this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
        ...
        this.loading = true
        const res = await this.$api.sys.deleteRole({ id: row._id })
        this.loading = false
        if (res.result) {
    ...
    async edit() {
      ...
      try {
        const res = await this.$api.sys.editRole({
          id: this.rowId,
          data: this.ruleForm
        })
        if (res.result) {
    ...
    async add() {
      const loading = this.localLoading()
      try {
        const res = await this.$api.sys.addRole(this.ruleForm)
        if (res.result) {
          this.centerDialogVisible = false
